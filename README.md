...
- **`okujiKobo/supabase/migrations/`** (web schema) — `002` through `028`, the real incremental history. Migration `027_asset_usage_check.sql` adds the `count_asset_references` RPC (SECURITY DEFINER) used by the assets hard-delete route. Migration `028_resize_cover_canvas.sql` rescales every cover element's coordinates after the cover canvas was resized from 560 × 392 → 1224 × 816 to match the inside-page artboard (612 × 792) per panel with 24px vertical fold bleed. No PostGIS (plain `lat`/`lng` + `geohash` text). Uses `institutions`, `acquisitions`, `completion_tokens`, `verification_tier`. `institutions` is never created by a migration — `026_institutions_rls.sql` documents that it was created in the Supabase dashboard, and that RLS on it was off in production until migration 026.
