...
export async function generateMetadata({
  ...
  if (!data) return { title: 'Passport · OkujiConnect' }
  return {
    title:       `${data.title} · OkujiConnect`,
  ...
  return (
    <section aria-labelledby={`section-${title.replace(/\s+/g, '-').toLowerCase()}`} className="space-y-3">
      <h2
        ...
        className="text-base font-semibold text-okuji-navy"
      >
        {title}
      </h2>
      {children}
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
              ...
              <p className="mt-1 text-sm text-okuji-gray-3">
                ...
                {institution ? (
                  <a
                    ...
                    className="text-okuji-navy font-medium hover:underline"
                  >
                    {institution.name}
                  </a>
                ) : creator ? (
        ...
        {/* ── Divider ──────────────────────────────────────────────────────── */}
        <hr className="my-8 border-okuji-gray-2" />
        ...
        <div className="space-y-10">
          ...
          {(passport.expected_spend_tier || !passport.is_free) && (
            <Section title="Expected spend">
              <div className="flex flex-wrap items-center gap-4 text-sm text-okuji-navy">
                {/* Passport price */}
                <div className="flex items-baseline gap-1.5">
                  <span className="font-semibold">
                    {passport.is_free ? 'Free' : priceDollars ? `$${priceDollars}` : 'Paid'}
                  </span>
                  <span className="text-okuji-gray-3 text-xs">passport</span>
                </div>
                ...
                {passport.expected_spend_tier && SPEND_TIER_LABELS[passport.expected_spend_tier] && (
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-okuji-gray-3 text-xs">on the ground</span>
                  </div>
                )}
              </div>
            </Section>
              ...
              <div className="space-y-3">
                {prizePages.map((page) => (
                  <div
                    key={page.id}
                    className={cn(
                      'flex items-start gap-3 rounded-panel border border-okuji-amber bg-amber-50 px-4 py-3'
                    )}
                  >
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-sm text-okuji-gray-3">
              ...
              <div className="flex items-start gap-4 rounded-panel border border-okuji-gray-2 bg-okuji-gray-1 p-4">
                ...
                <div className="min-w-0">
                  ...
                  {creator && (
                    <a
                      href={`/creator/${creator.id}`}
                      className="mt-2 inline-block text-xs font-medium text-okuji-teal hover:underline"
                    >
                      View all passports →
                    </a>
                  )}
          ...
          {/* ── Bottom acquire CTA ──────────────────────────────────────────── */}
          <div className="flex justify-center border-t border-okuji-gray-2 pt-8">
            <AcquireButton
              passportId={passport.id}
              isFree={passport.is_free}
              priceCents={passport.price_cents}
