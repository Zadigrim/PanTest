...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: stop ? `${stop.name} — Stop Library` : 'Stop — PanoplyConnect' }
}
...
function ClassifierBadge({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center rounded-card bg-panoply-teal-lt px-2.5 py-0.5 text-xs font-medium text-panoply-teal-dk capitalize">
      {label.replace(/_/g, ' ')}
    </span>
  )
}
...
function InfoRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1 sm:flex-row sm:gap-4">
      <dt className="text-xs font-semibold text-panoply-gray-3 uppercase tracking-wide sm:w-36 shrink-0">
        {label}
      </dt>
      <dd className="text-sm text-panoply-navy">{children}</dd>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-panoply-gray-1">
      ...
      <main className="mx-auto max-w-3xl px-8 py-10">
        {/* Stop hero */}
        <div className="bg-white rounded-modal border border-panoply-gray-2 p-8 mb-6">
          {/* Icon + name */}
          <div className="flex items-center gap-4 mb-6">
            <div
              className="flex h-16 w-16 shrink-0 items-center justify-center rounded-panel bg-panoply-gray-1 text-4xl"
              aria-hidden="true"
            >
              {stopData.stamp_icon ?? '📍'}
            </div>
            <div>
              ...
              {location && (
                <p className="text-sm text-panoply-gray-3 mt-0.5">{location}</p>
              )}
            </div>
          </div>
              ...
              <InfoRow label="Grade levels">
                <div className="flex flex-wrap gap-1.5">
                  {gradeLevels.map((g) => (
                    <span
                      key={g}
                      className="inline-flex items-center rounded-card bg-panoply-gray-2 px-2 py-0.5 text-xs font-medium text-panoply-navy"
                    >
                      {g}
                    </span>
                  ))}
              ...
              <InfoRow label="Subject areas">
                <div className="flex flex-wrap gap-1.5">
                  {subjectAreas.map((s) => (
                    <span
                      key={s}
                      className="inline-flex items-center rounded-card bg-panoply-gray-2 px-2 py-0.5 text-xs font-medium text-panoply-navy capitalize"
                    >
                      {s.replace(/_/g, ' ')}
                    </span>
                  ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div className="bg-white rounded-panel border border-panoply-gray-2 p-5">
            <p className="text-xs font-semibold text-panoply-gray-3 uppercase tracking-wide mb-1">
              Acknowledgments
            </p>
            <p className="text-3xl font-bold tabular-nums text-panoply-navy">
              {acknowledgments}
            </p>
          </div>
          <div className="bg-white rounded-panel border border-panoply-gray-2 p-5">
            ...
            {acknowledgments > 0 && (
              <p className="text-xs text-panoply-gray-3 mt-1">
        ...
        {(creatorName ?? institutionName) && (
          <div className="bg-white rounded-panel border border-panoply-gray-2 px-5 py-4 mb-8">
        ...
        {/* Import CTA */}
        <div className="bg-panoply-teal/5 border border-panoply-teal-lt rounded-panel p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
          ...
          <Link
            ...
            className="shrink-0 inline-flex items-center h-9 px-5 rounded-panel bg-panoply-teal text-white text-sm font-medium hover:bg-[#0F6E56] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal"
          >
            Import this stop
          </Link>
        </div>
