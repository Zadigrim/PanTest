...
export default async function LibraryPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <h1 className="mb-6 font-serif text-2xl font-bold text-panoply-navy">My Library</h1>
      ...
      {(acquisitions ?? []).length === 0 && (
        <div className="rounded-panel border border-dashed border-panoply-gray-2 px-8 py-16 text-center">
          <p className="text-4xl">📖</p>
          <p className="mt-3 text-panoply-gray-3">No passports yet.</p>
          <Link href="/" className="mt-4 inline-block text-sm font-semibold text-panoply-teal hover:underline">
            Browse passports →
          </Link>
        </div>
      )}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-8">
      <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-panoply-gray-3">{title}</h2>
...
function LibraryRow({
  ...
  return (
    <div className="flex items-center gap-4 rounded-panel border border-panoply-gray-2 bg-white p-4 shadow-sm">
      <div
      ...
      <div className="flex-1 min-w-0">
        <p className="truncate font-serif font-semibold text-panoply-navy">{passport.title}</p>
        {completed ? (
          <p className="text-xs text-panoply-teal">
          ...
          <>
            ...
            <div className="mt-1.5 h-1.5 w-full rounded-full bg-panoply-gray-2">
              <div
      ...
      <div className="flex shrink-0 gap-2">
        <Link
          ...
          className="rounded-card bg-panoply-teal px-3 py-1.5 text-xs font-semibold text-white hover:bg-panoply-teal-dk transition-colors"
        >
