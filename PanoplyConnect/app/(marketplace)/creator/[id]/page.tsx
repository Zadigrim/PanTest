  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-8 flex items-start gap-5">
        <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-panoply-teal-lt text-3xl">
          {creator.avatar_url
            ? <img src={creator.avatar_url} alt={displayName} className="h-20 w-20 rounded-full object-cover" />
            : '🧭'}
        </div>
        <div>
          <h1 className="font-serif text-2xl font-bold text-panoply-navy">{displayName}</h1>
          {institution && (
            <p className="text-sm text-panoply-gray-3">Institutional creator</p>
          )}
          {creator.bio && (
            <p className="mt-2 max-w-2xl text-sm text-panoply-gray-3">{creator.bio}</p>
          )}
          {(avgMood !== null || avgCompletion !== null) && (
            <div className="mt-2 flex gap-4 text-sm text-panoply-gray-3">
      ...
      {(passports ?? []).length === 0 ? (
        <p className="py-8 text-center text-panoply-gray-3">No published passports yet.</p>
      ) : (
