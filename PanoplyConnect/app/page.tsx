...
import type { PanoplyConnectRole } from '@/lib/roles'

// ─── Role-access rules ────────────────────────────────────────────────────────

const SECTION_ACCESS: Record<string, PanoplyConnectRole[]> = {
...
const NAV_CARDS = [
  {
    ...
    requiresRole: null as PanoplyConnectRole | null,
  },
  {
    key:     'manage',
    ...
    requiresRole: 'institutional_manager' as PanoplyConnectRole,
  },
  {
    key:     'access',
...
function MetricCard({
  ...
  const inner = (
    <div className="rounded-panel border border-panoply-gray-2 bg-white p-5 transition-shadow hover:shadow-sm">
      <p className="text-xs font-medium uppercase tracking-wide text-panoply-gray-3 mb-1">
        {label}
      </p>
      <p className="text-3xl font-bold tabular-nums text-panoply-navy">{value}</p>
      {sub && <p className="mt-1 text-xs text-panoply-gray-3">{sub}</p>}
    </div>
  )

  if (href) {
    return (
      <Link href={href} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal rounded-panel">
        {inner}
      </Link>
    )
  }
...
function AlertCard({
  ...
  return (
    <div className="flex items-start gap-3 rounded-panel border border-panoply-amber bg-amber-50 px-4 py-3">
      <span className="text-lg leading-none mt-0.5 shrink-0" aria-hidden="true">⚠</span>
      <div className="flex-1 min-w-0">
        <Link
          href={href}
          className="text-sm font-medium text-panoply-navy hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal rounded-sm"
        >
          {message}
        </Link>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        ...
        {/* ── Page header ──────────────────────────────────────────────────── */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-panoply-navy">
            {displayName ? `Welcome back, ${displayName}` : 'PanoplyConnect'}
          </h1>
          <p className="mt-1 text-sm text-panoply-gray-3">
            {ROLE_LABELS[activeRole]}
            {roles.length > 1 && (
              <span className="ml-2 text-panoply-gray-3">
        ...
        {!hasAnyData ? (
          <div className="mb-8 rounded-panel border border-dashed border-panoply-gray-2 px-6 py-10 text-center">
            <p className="text-lg font-semibold text-panoply-navy">
              Welcome to PanoplyConnect.
            </p>
            <p className="mt-1 text-sm text-panoply-gray-3">
            ...
            <Link
              href="/design"
              className="mt-4 inline-flex items-center justify-center rounded-panel bg-panoply-teal px-5 h-9 text-sm font-medium text-white hover:bg-panoply-teal-dk transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal"
            >
              Create a passport
            </Link>
          </div>
        ...
        <section aria-label="Navigation">
          <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-panoply-gray-3">
            Sections
          </h2>
            ...
            {NAV_CARDS.map((card) => {
              ...
              return (
                <Link
                  ...
                  className={`
                    block rounded-card border border-panoply-gray-2 p-5
                    transition-shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal
                    ${hasAccess
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <p className="font-semibold text-panoply-navy">{card.title}</p>
                    {hasPendingAlert && (
                      <span
                        className="shrink-0 rounded-full bg-panoply-amber px-2 py-0.5 text-xs font-semibold text-white"
                  ...
                  <p className="mt-1 text-sm text-panoply-gray-3">{card.description}</p>
                </Link>
              )
            })}
          </div>
