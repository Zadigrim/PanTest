  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <button
        onClick={handleClick}
        disabled={isPending}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-panel text-xs font-medium bg-panoply-teal text-white hover:bg-[#0F6E56] disabled:opacity-50 disabled:pointer-events-none transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal"
      >
        {isPending ? 'Saving…' : '✓ Mark distributed'}
      </button>
      {error && <span className="text-xs text-panoply-coral">{error}</span>}
    </div>
  )
}
...
function TokenStatusBadge({
  ...
  if (token.prize_distributed) {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-panoply-teal-lt text-panoply-teal">
        Distributed
      </span>
    )
  }
  if (token.redeemed_at) {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-panoply-amber/20 text-panoply-amber">
        Redeemed
      </span>
    )
  }
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-panoply-gray-2 text-panoply-gray-3">
      Pending
    </span>
  )
}
...
export default function PassportAnalyticsPage() {
  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center min-h-64">
        <p className="text-panoply-gray-3 text-sm animate-pulse">Loading analytics…</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-8 max-w-6xl space-y-10">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-panoply-navy">{passportTitle}</h1>
        <p className="text-sm text-panoply-gray-3 mt-1">Analytics · Last 30 days</p>
      </div>

      {/* ── 1. Stop engagement table ─────────────────────────────────────── */}
      <section>
        <h2 className="text-base font-semibold text-panoply-navy mb-3">Stop engagement</h2>
        {analytics && analytics.stopEngagement.length > 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-panoply-gray-2 bg-panoply-gray-1">
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Stop name</th>
                  <th className="px-4 py-3 text-right font-medium text-panoply-gray-3">Stamps</th>
                  <th className="px-4 py-3 text-right font-medium text-panoply-gray-3">Avg dwell</th>
                  <th className="px-4 py-3 text-right font-medium text-panoply-gray-3">Return visits</th>
                  <th className="px-4 py-3 text-right font-medium text-panoply-gray-3">Mood rating</th>
                </tr>
              </thead>
              <tbody>
                {analytics.stopEngagement.map((stop, i) => (
                  <tr
                    key={stop.stopId}
                    className={`border-b border-panoply-gray-2 last:border-0 ${
                      i % 2 === 1 ? 'bg-panoply-gray-1/40' : ''
                    }`}
                  >
                    <td className="px-4 py-3 font-medium text-panoply-navy">{stop.stopName}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-panoply-navy">
                      {stop.stampCount}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums text-panoply-gray-3">
          ...
          <p className="text-sm text-panoply-gray-3">No stop data available yet.</p>
        )}
      </section>

      {/* ── 2. Completions bar chart ─────────────────────────────────────── */}
      <section>
        <h2 className="text-base font-semibold text-panoply-navy mb-3">Completions per day</h2>
        <div className="bg-white rounded-panel border border-panoply-gray-2 p-4">
          <CompletionsBarChart buckets={dayBuckets} />
          <p className="text-xs text-panoply-gray-3 mt-2 text-right">Last 30 days</p>
        </div>
      </section>

      {/* ── 3. Token redemption log ──────────────────────────────────────── */}
      <section>
        <h2 className="text-base font-semibold text-panoply-navy mb-3">Token redemption log</h2>
        {tokens.length > 0 ? (
          <div className="bg-white rounded-panel border border-panoply-gray-2 overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-panoply-gray-2 bg-panoply-gray-1">
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Token code</th>
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Collector</th>
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Generated</th>
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Status</th>
                </tr>
              </thead>
              <tbody>
                {tokens.map((t, i) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-xs text-panoply-navy">{t.token_code}</td>
                    <td className="px-4 py-3 text-panoply-navy">
                      {t.collectorName ?? <span className="text-panoply-gray-3 italic">Unknown</span>}
                    </td>
                    <td className="px-4 py-3 text-panoply-gray-3 tabular-nums text-xs">
          ...
          <p className="text-sm text-panoply-gray-3">No tokens generated yet.</p>
        )}
      </section>
      ...
      <section>
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-base font-semibold text-panoply-navy">Distribution pending</h2>
          {pendingTokens.length > 0 && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-panoply-amber/20 text-panoply-amber">
        ...
        {pendingTokens.length === 0 ? (
          ...
          <div className="bg-white rounded-panel border border-panoply-amber overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-panoply-gray-2 bg-panoply-amber/5">
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Token code</th>
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Collector</th>
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Generated</th>
                  <th className="px-4 py-3 text-left font-medium text-panoply-gray-3">Action</th>
                </tr>
              </thead>
              <tbody>
