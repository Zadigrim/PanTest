...
function PagePrizeForm({
  ...
  return (
    <div className="bg-white rounded-panel border border-panoply-gray-2 p-5">
      {/* Page header */}
      <div className="mb-4">
        <p className="text-xs text-panoply-gray-3 font-medium uppercase tracking-wide">
          {page.passport_title}
        </p>
        <h3 className="text-sm font-semibold text-panoply-navy mt-0.5">{pageLabel}</h3>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            Prize value{' '}
            <span className="font-normal text-panoply-gray-3">(optional, USD)</span>
          </label>
          <div className="relative w-44">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-panoply-gray-3 text-sm select-none pointer-events-none">
              $
            </span>
            <input
              id={`${formId}-value`}
              ...
              placeholder="0.00"
              className="h-9 w-full rounded-panel border border-panoply-gray-2 bg-panoply-gray-1 pl-7 pr-3 text-sm text-panoply-navy placeholder:text-panoply-gray-3 focus:outline-none focus:ring-2 focus:ring-panoply-teal focus:border-panoply-teal transition-colors"
            />
          </div>
        </div>
        ...
        <fieldset>
          ...
          <div className="space-y-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-panoply-navy">All locations</span>
            </label>
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="accent-panoply-teal"
              />
              <span className="text-sm text-panoply-navy">Specific stops</span>
            </label>

            {/* Stop checkboxes (only shown in specific mode) */}
            {form.location_mode === 'specific' && page.stops.length > 0 && (
              <div className="ml-6 mt-2 space-y-2 border-l-2 border-panoply-gray-2 pl-4">
                {page.stops.map((stop) => (
                  <label
                    key={stop.id}
                  ...
                  >
                    <input
                      ...
                      className="accent-panoply-teal"
                    />
                    <span className="text-sm text-panoply-navy">{stop.name}</span>
                  </label>
                ))}
              </div>
            )}
            ...
            {form.location_mode === 'specific' && page.stops.length === 0 && (
              <p className="ml-6 text-xs text-panoply-gray-3">
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          {saved && (
            <span className="text-sm text-panoply-teal font-medium">Saved ✓</span>
          )}
          {saveError && (
            <span role="alert" className="text-sm text-panoply-coral">
              {saveError}
            </span>
          )}
        </div>
...
export default function PrizesPage() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-panoply-navy">Prize configuration</h1>
