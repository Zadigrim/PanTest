...
function TabNav({ current }: { current: AssetType }) {
  return (
    <nav
      className="flex gap-0.5 border-b border-panoply-gray-2 mb-8"
      ...
      {ASSET_TYPES.map((type) => (
        <Link
          ...
          className={cn(
            ...
            current === type
              ? 'bg-white border border-b-white border-panoply-gray-2 text-panoply-navy'
...
function AssetCard({
  ...
  return (
    <div className="bg-white rounded-panel border border-panoply-gray-2 overflow-hidden group">
      {/* Preview area */}
      <div className="aspect-video bg-panoply-gray-1 flex items-center justify-center">
        {asset.url ? (
          ...
          <span className="text-3xl text-panoply-gray-2" aria-hidden="true">
            🖼
          </span>
        )}
      </div>
      ...
      <div className="px-3 py-2.5 flex items-center justify-between gap-2">
        <p className="text-sm font-medium text-panoply-navy truncate">
          {asset.name ?? 'Untitled'}
        </p>
        {isShared && (
          <span className="shrink-0 inline-flex items-center rounded-card bg-panoply-teal-lt px-2 py-0.5 text-xs font-medium text-panoply-teal-dk">
            Shared
          </span>
        )}
      </div>
...
function EmptyState({ meta }: { meta: (typeof TYPE_META)[AssetType] }) {
  return (
    <div className="rounded-modal border-2 border-dashed border-panoply-gray-2 py-20 text-center">
      <div
        className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-panoply-gray-1 text-3xl"
        aria-hidden="true"
      >
        🖼
      </div>
      <h2 className="text-base font-semibold text-panoply-navy">{meta.emptyHeading}</h2>
      <p className="mt-2 text-sm text-panoply-gray-3 max-w-xs mx-auto">{meta.emptyBody}</p>
    </div>
  )
}
...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: `${TYPE_META[type].label} — PanoplyConnect Assets` }
}
    ...
    <div className="min-h-screen bg-panoply-gray-1">
      ...
      <header className="border-b border-panoply-gray-2 bg-white px-8 py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-xl font-bold text-panoply-navy">Assets</span>
          </div>
          <Link
            href="/design"
      ...
      <main className="mx-auto max-w-6xl px-8 py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-lg font-semibold text-panoply-navy">{meta.label}</h2>
