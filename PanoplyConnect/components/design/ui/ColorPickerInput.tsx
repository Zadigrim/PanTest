...
export function ColorPickerInput({ value, onChange, onCommit }: Props) {
  ...
  return (
    <div className="relative">
      ...
      <button
        type="button"
        className="h-8 w-8 shrink-0 rounded-card border border-panoply-gray-2 focus:outline-none focus:ring-2 focus:ring-panoply-teal"
      ...
      {open && (
        <div
          ref={popoverRef}
          className="absolute right-0 top-10 z-50 w-64 rounded-panel border border-panoply-gray-2 bg-white shadow-xl"
        ...
        >
          ...
          <div className="space-y-2 p-3">
            ...
            <div className="flex items-center gap-2">
              <span className="w-3 shrink-0 text-[10px] text-panoply-gray-3">H</span>
              <input
              ...
              <input
                ...
                className="h-6 w-12 rounded border border-panoply-gray-2 px-1 text-center font-mono text-[10px]"
              />
            </div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {([['R', r, 0], ['G', g, 1], ['B', b, 2]] as [string, number, number][]).map(([lbl, val]) => (
                <div key={lbl} className="space-y-0.5">
                  <label className="block text-center text-[10px] text-panoply-gray-3">{lbl}</label>
                  <input
                    ...
                    className="h-6 w-full rounded border border-panoply-gray-2 px-1 text-center font-mono text-[10px]"
                  />
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <input
                ...
                className="h-7 flex-1 rounded border border-panoply-gray-2 px-2 font-mono text-sm uppercase text-panoply-navy focus:border-panoply-teal focus:outline-none"
                placeholder="0D1B2A"
              />
              <div className="h-7 w-7 shrink-0 rounded border border-panoply-gray-2" style={{ backgroundColor: `#${hex}` }} />
            </div>

            {/* Recent colors */}
            {recentColors.length > 0 && (
              <div>
                <p className="mb-1 text-[10px] text-panoply-gray-3">Recent colors</p>
                <div className="flex flex-wrap gap-1">
                  {recentColors.map((rc) => (
                    <button
                      key={rc}
                      type="button"
                      className="h-5 w-5 rounded border border-panoply-gray-2 transition-transform hover:scale-110"
            ...
            <button
              type="button"
              onClick={close}
              className="mt-1 h-7 w-full rounded-card bg-panoply-teal text-xs font-medium text-white hover:bg-panoply-teal-dk transition-colors"
            >
              Done
            </button>
          </div>
