...
export function PrintPassportModal({ passport, onClose }: PrintPassportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative z-10 flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden rounded-modal border border-panoply-gray-2 bg-white shadow-xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-panoply-gray-2 px-6 py-4">
          <h2 className="text-base font-semibold text-panoply-navy">Print Physical Passports</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="space-y-0.5">
            <p className="text-sm text-panoply-navy">
          ...
          {/* Stop selection */}
          <section>
            <p className="mb-2 text-sm font-medium text-panoply-navy">Select stops to include:</p>
            {loading && (
              <p className="text-sm text-panoply-gray-3 animate-pulse">Loading stops…</p>
            )}
            {loadError && (
              <p role="alert" className="text-sm text-panoply-coral">{loadError}</p>
            )}
            {!loading && !loadError && stops.length === 0 && (
              <p className="text-sm text-panoply-gray-3">No stops found for this passport.</p>
            )}
            {!loading && !loadError && stops.length > 0 && (
                ...
                {stops.map((stop, i) => (
                  <label key={stop.id} className="flex cursor-pointer items-center gap-3">
                    ...
                    <span className="text-sm text-panoply-navy">
                      <span className="text-panoply-gray-3 mr-1">Stop {i + 1} —</span>
                      {stop.name}
                    </span>
                  </label>
                ))}
          ...
          {/* Copies */}
          <section>
            <label htmlFor={copiesId} className="mb-1.5 block text-sm font-medium text-panoply-navy">
              Number of copies
            </label>
            <input
              id={copiesId}
              ...
              className="h-9 w-28 rounded-panel border border-panoply-gray-2 px-3 text-sm text-panoply-navy focus:outline-none focus:ring-2 focus:ring-panoply-teal"
            />
          </section>

          {/* Sheet calculation */}
          {!loading && selectedIds.size > 0 && (
            <div className="rounded-panel bg-panoply-teal-lt px-4 py-3">
              <p className="text-sm text-panoply-teal-dk">
              ...
              <p className="mt-0.5 text-sm text-panoply-teal-dk/80">
          ...
          {/* Journal override */}
          <section>
            <p className="mb-2 text-sm font-medium text-panoply-navy">Journal lines:</p>
            <div className="space-y-2">
              {(
                [
          ...
          {genError && (
            <p role="alert" className="text-sm text-panoply-coral">{genError}</p>
          )}
        </div>

        {/* Footer */}
        <div className="flex gap-3 border-t border-panoply-gray-2 px-6 py-4">
          <button
            onClick={() => void handleGenerate('preview')}
            disabled={generating || loading || selectedIds.size === 0}
            className="flex-1 rounded-panel border border-panoply-gray-2 bg-white py-2 text-sm font-medium text-panoply-navy hover:border-panoply-teal hover:text-panoply-teal-dk transition-colors disabled:opacity-50 disabled:pointer-events-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal"
          >
          ...
          <button
            ...
            disabled={generating || loading || selectedIds.size === 0}
            className="flex-1 rounded-panel bg-panoply-teal py-2 text-sm font-medium text-white hover:bg-panoply-teal-dk transition-colors disabled:opacity-50 disabled:pointer-events-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-panoply-teal"
          >
