...
export function AdminPanel({ userEmail }: Props) {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-[200] flex flex-col items-start">
      {/* Floating panel */}
      {open && (
        <div className="mb-2 w-72 rounded-modal border border-panoply-gray-2 bg-white shadow-2xl">
          {/* Header */}
          <div className="border-b border-panoply-gray-2 px-4 py-3">
            <p className="text-sm font-semibold text-panoply-navy">⚙ Admin Panel</p>
            <p className="mt-0.5 text-xs text-panoply-gray-3 truncate">
          ...
          <div className="space-y-4 px-4 py-3">
            {/* Simulate role */}
            <div>
              <p className="mb-2 text-xs font-semibold text-panoply-navy">Simulate role</p>
            ...
            {isInstitutional && (
              <div>
                <p className="mb-1.5 text-xs font-semibold text-panoply-navy">
                  Institution type
                </p>
                <select
                  value={pendingInstType}
                  onChange={(e) => setPendingInstType(e.target.value as InstitutionType)}
                  className="w-full rounded-panel border border-panoply-gray-2 px-2 py-1.5 text-sm text-panoply-navy focus:outline-none focus:ring-2 focus:ring-panoply-teal"
                >
            ...
            {/* Actions */}
            <div className="flex gap-2 border-t border-panoply-gray-2 pt-3">
              <button
                onClick={handleApply}
                className="flex-1 rounded-panel bg-panoply-teal px-3 py-1.5 text-sm font-medium text-white hover:bg-panoply-teal-dk transition-colors"
              >
                Apply
              </button>
              <button
                onClick={handleReset}
                className="flex-1 rounded-panel border border-panoply-gray-2 px-3 py-1.5 text-sm font-medium text-panoply-navy hover:border-panoply-teal transition-colors"
              >
                Reset
              </button>
            </div>
            ...
            {simulation.active && (
              <p className="text-center text-xs font-medium text-panoply-amber">
      ...
      <button
        ...
        className={`flex h-9 w-9 items-center justify-center rounded-full text-base shadow-md transition-colors ${
          simulation.active
            ...
            : 'bg-panoply-navy text-white hover:bg-panoply-teal'
        }`}
      >
        ⚙
      </button>
