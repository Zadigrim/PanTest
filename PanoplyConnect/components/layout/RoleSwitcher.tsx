...
export function RoleSwitcher({ roles, activeRole }: RoleSwitcherProps) {
  ...
  function handleChange(e: React.ChangeEvent<HTMLSelectElement>) {
    const next = e.target.value as PanoplyConnectRole
    startTransition(() => {
      ...
      <div className="relative">
        <select
          ...
          className="
            ...
            hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-panoply-teal
            disabled:opacity-50 transition-colors
          "
        >
          {roles.map((r) => (
            <option key={r} value={r} className="bg-panoply-navy text-white">
              {ROLE_LABELS[r]}
            </option>
          ))}
        </select>
