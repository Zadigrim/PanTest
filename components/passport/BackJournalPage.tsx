...
//
// Row-flow layout: each stamped stop is one ROW (BackJournalRow). Rows are
// packed onto pages by estimated height (see lib/back-pages-layout) so a
// baseline-only stop is a short card, not a whole page. A page (BackJournalPage)
// stacks the rows packed onto it inside a ScrollView — the safety net that
// absorbs estimate drift or a single over-tall row without clipping.
//
//   - BackJournalCover: the divider that opens the journal section.
//   - BackJournalRow:   one stamped stop's record, as a card.
//   - BackJournalPage:  one page = the rows packed onto it.
//
// PRIVATE to the collector. Data is assembled by useBackPages from
...
import {
  BACK_PAGE_PAD_X, BACK_PAGE_PAD_TOP, BACK_PAGE_PAD_BOTTOM, ROW_PAD, ROW_GAP,
} from '../../lib/back-pages-layout'
...
// One stamped stop, rendered as a self-contained card so stacked rows read as
// separate stops. Same content as before — only the framing changed.
export function BackJournalRow({ record }: { record: BackPageRecord }) {
  const { stop, stamp, verifiedAt, journalBody, mood, photos, review } = record
  const hasContent = !!journalBody || photos.length > 0 || !!review

  return (
    <View style={styles.row}>
      {/* Baseline header (always): stamp mark + stop name/location + visited time. */}
...
// One page = the rows packed onto it (see lib/back-pages-layout.packBackPages).
// The ScrollView is the estimate-drift safety net: packing targets the visible
// budget to minimize pages, but nothing clips if an estimate is low or a row
// is taller than a page. Vertical scroll coexists with the flipper's swipe.
export function BackJournalPage({ records }: { records: BackPageRecord[] }) {
  const { pageW, pageH } = usePageDimensions()
  return (
    <View style={[styles.page, { width: pageW, height: pageH, backgroundColor: PAPER }]}>
      <GuillocheBackground color={GOLD} opacity={0.05} width={pageW} height={pageH} />
      <ScrollView contentContainerStyle={styles.pageBody} showsVerticalScrollIndicator={false}>
        {records.map((r) => <BackJournalRow key={r.stopId} record={r} />)}
      </ScrollView>
    </View>
  )
}
...
const styles = StyleSheet.create({
  ...
  // Page (stack of rows) — padding mirrors lib/back-pages-layout constants.
  pageBody: {
    paddingHorizontal: BACK_PAGE_PAD_X,
    paddingTop: BACK_PAGE_PAD_TOP,
    paddingBottom: BACK_PAGE_PAD_BOTTOM,
  },

  // Row card — padding + gap mirror the layout constants so estimates match.
  row: {
    padding: ROW_PAD,
  ...
  stampWrap: { width: 56, height: 56, alignItems: 'center', justifyContent: 'center' },
  headText: { flex: 1 },
  stopName: { fontFamily: 'serif', fontSize: 18, fontWeight: '700', color: INK },
  locationName: { fontSize: 12, color: palette.muted, marginTop: 2 },
  visited: { fontSize: 12, color: GOLD, fontWeight: '600', marginTop: 3, letterSpacing: 0.3 },

  photoStrip: { marginTop: 14 },
  photo: { width: 120, height: 120, borderRadius: 8, marginRight: 10, backgroundColor: '#e6dfce' },

  journalBody: { fontSize: 15, lineHeight: 23, color: INK, fontFamily: 'serif', marginTop: 10 },
  ...
  emptyNote: { fontSize: 13, color: palette.muted, fontStyle: 'italic', marginTop: 10 },
})
