...
import { SPEND_TIERS, spendTierLabel } from '@/lib/utils/spend-tiers'
import type { SpendTier, CreatorDecision } from '@/lib/supabase/types'
...
export function PassportSettingsPanel({ onClose }: Props) {
  ...
  const handleVerifySpend = async () => {
    ...
    try {
      const res = await fetch('/api/verify-spend', {
        ...
        body: JSON.stringify({ passportId: passport.id, requestedTier: passport.expected_spend_tier }),
      })
  ...
  const handleDecision = async (d: CreatorDecision) => {
    ...
    const { data: latest } = await supabase
      ...
      .single()
    if (latest) {
      await supabase.from('spend_verification_log').update({ creator_decision: d }).eq('id', latest.id)
    }
  }
...
function VerifyResultCard({
  ...
  return (
    <div className={`rounded-panel border px-3 py-3 text-sm space-y-2 ${result.tier_match ? 'border-panoply-teal bg-panoply-teal-lt' : 'border-panoply-amber bg-panoply-amber/10'}`}>
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className={`font-semibold ${result.tier_match ? 'text-panoply-teal-dk' : 'text-panoply-amber'}`}>
            {result.tier_match ? '✓ Tier confirmed' : '⚠ Tier mismatch'}
          </p>
          <p className="text-xs text-panoply-gray-3">
            AI suggests: {spendTierLabel(result.suggested_tier)} (${result.range_low}–${result.range_high})
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2 border-t border-panoply-gray-2 pt-2">
        ...
        {!result.tier_match && (
          <Button size="sm" variant="secondary" className="flex-1 text-xs" onClick={() => onDecision('adjusted')}>
            Use AI suggestion
          </Button>
        )}
        <Button size="sm" variant="ghost" className="flex-1 text-xs" onClick={() => onDecision('overridden')}>
          Keep mine
        </Button>
      </div>
    </div>
